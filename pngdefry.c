...
/*	Swap BGR to RGB, BGRA to RGBA */
	...
	/*	Force VERY conservative repacking size ... */		
		...
		repack_length = tdefl_compress_mem_to_mem(data_repack+4, repack_size-4, data_out, out_length, TDEFL_WRITE_ZLIB_HEADER | s_tdefl_num_probes[flag_compression]);
		if (repack_length == 0)
		{
			free (data_out);
			free (data_repack);
...
int main (int argc, char **argv)
{
	...
	if (argc == 1)
	{
		...
		printf ("  -c(value)  specify compression level [1-10] (defaults to 6)\n");
		return 0;
	}
	...
	for (i=1; i<argc; i++)
	{
		...
		switch (argv[i][1])
		{
			...
			case 'c': 
			  ...
			  {
			    char *endptr;
					flag_compression = strtol(argv[i]+2, &endptr, 10);
					if (*endptr || flag_compression >10)
					{
						printf ("pngdefry : invalid compression '%s'\n", argv[i]+2);
						return -1;
					} else {
            printf("setting compression to %d\n", flag_compression);
					}
			  }
				argv[i][2] = 0;
        break;
